...
const setPersistentStats = async localStats => {
  ...
  try {
    ...
    if (e.message === '409') {
      ...
      return await delay(
        () =>
          upload(token, hook, mergedStatsEncoded, statsFilename, '', sha, updateStatsMsg, 'Stats'),
  ...
  if (
    ...
    setTimeout(function () {
      ...
      if (
        ...
        const currentDiff = document.querySelector('div[diff]')?.innerText || 'Unrated';
        updateReadmeWithDiscussionPost(
          addition,
          problemName,
          readmeFilename,
          discussionMsg,
